...
RSpec.describe 'Collection', type: :feature do
  example 'Guest shows infinite activities feed' do
    ...
    wait_until_loaded
    resource_selector(playwright_page.url).locator(':nth-match(.CID-Card, 10)')
    click_button('Load more')
    wait_until_loaded
    resource_selector(playwright_page.url).locator(':nth-match(.CID-Card, 20)')
  end
  ...
  example 'Guest shows paginated activities feed' do
    ...
    wait_for { page }.to have_content('Activities')
    wait_until_loaded
    collection_iri = playwright_page.url
