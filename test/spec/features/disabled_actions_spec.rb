...
RSpec.describe 'Disabled actions', type: :feature do
  ...
  context 'expired question' do
    ...
    let(:vote_buttons_expectation) do
      wait_for { playwright_page.locator('.Button[disabled][title="Voting is no longer possible"]').count }.to eq 3
    end
  ...
  context 'motion of expired question' do
    ...
    let(:vote_buttons_expectation) do
      wait_for { playwright_page.locator('.Button[disabled][title="Voting is no longer possible"]').count }.to be >= 3
    end
