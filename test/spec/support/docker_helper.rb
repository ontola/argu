...
module DockerHelper
  MANAGED_DATABASES = %i[apex email token]
...
END_HEREDOC
  ...
  def docker_reset_redis
    ...
    redis_cache_persisted_client.flushdb
  ...
  def docker_container(name)
    docker_container_find("argu_#{name}_1") || docker_container_find("argu-#{name}-1")
  end
  ...
  def docker_exec(service, commands)
    ...
    result = Timeout.timeout(240, Timeout::Error, "Execution of #{commands} expired for service #{service}") do
  ...
  def run_local(service, commands)
    path = File.expand_path("../services/#{service}")
