...
RSpec.describe 'Cache', type: :system do
  ...
  context 'without language' do
    it 'fetches the ontology' do
      ...
      expect_cached_triple(tenantized_ontology, RDF::RDFV[:type], RequestsHelper::ONTOLA[:Ontology])
    end

    it 'Serves all languages without language header' do
      bulk tenantized_ontology

      expect_cached_triple('https://argu.co/ns/core#ProArgument', RDF::RDFS[:label], RDF::Literal('Voordeel', language: 'nl'))
      expect_cached_triple('https://argu.co/ns/core#ProArgument', RDF::RDFS[:label], RDF::Literal('Pro', language: 'en'))
    end
    ...
    it 'Serves motion form without language header' do
      ...
      expect_cached_triple(motion_form, RDF::RDFV[:type], RequestsHelper::FORM[:Form])
    ...
    it 'Serves motion new action without language header' do
      ...
      expect_uncached_triple(motion_new, RDF::RDFV[:type], RequestsHelper::ONTOLA['Create::Motion'])
      expect_uncached_triple(motion_new, RDF::Vocab::SCHEMA.name, RDF::Literal('New idea'))
    end
  ...
  context 'with language' do
    it 'Serves motion new action in user language' do
      ...
      expect_uncached_triple(motion_new, RDF::RDFV[:type], RequestsHelper::ONTOLA['Create::Motion'])
      expect_uncached_triple(motion_new, RDF::Vocab::SCHEMA.name, RDF::Literal('Nieuw idee'))
    end
    ...
    it 'Serves motion form in user language' do
      ...
      expect_uncached_triple(motion_form, RDF::RDFV[:type], RequestsHelper::FORM[:Form])
  ...
  def expect_cached_triple(subject, predicate, object)
    expect_triple(subject, predicate, object, nil)
  end

  def expect_uncached_triple(subject, predicate, object)
    expect_triple(subject, predicate, object)
  end
end
